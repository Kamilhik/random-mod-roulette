...
export function HistoryList({ items, language }: HistoryListProps) {
  const t = UI_TEXT[language];

  return (
    <aside className="rounded-lg border border-white/10 bg-zinc-950/72 p-4 shadow-2xl shadow-black/30 backdrop-blur xl:sticky xl:top-6">
      <div className="mb-4 flex items-center gap-2">
        <Clock3 className="size-5 text-emerald-300" aria-hidden="true" />
        <h2 className="text-lg font-black text-white">{t.history.title}</h2>
      </div>
            ...
            <li key={`${mod.id}-${index}`} className="group flex items-center gap-3 rounded-md border border-white/10 bg-white/[0.035] p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-zinc-400">
                  {mod.author} · {formatDownloads(mod.downloads, language)}
                </p>
              </div>
              <a
                href={mod.url}
                ...
                aria-label={`${t.history.open}: ${mod.title}`}
              >
